...
namespace AdeCartAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(UserManager<User> user, IMapper mapper, SignInManager<User> login, IPasswordHasher<User> passwordHasher, Credentials _credentials)
        {
            this.user = user;
            this.mapper = mapper;
            this.login = login;
            ...
            try
            {
                ...
                if (passwordVerifyResult.ToString() == "Success")
                {
                   // await login.SignInAsync(currentUser, false);
                    ...
                    return Ok(token);
                    /* 
                     await login.CreateUserPrincipalAsync(currentUser);return this.StatusCode(StatusCodes.Status200OK, $"Welcome,{currentUser.UserName} Shop Shop Shop");*/
                }
